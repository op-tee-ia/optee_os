...
static int self_test_division(void)
{
	...
	a = (signed int)((unsigned int)(vaddr_t)&a & 0xFFFFF);
	b = (signed int)((unsigned int)(vaddr_t)&b & 0x00FFF) + 1;
	c = a / b;
	d = a % b;
	r = ((b * c + d) == a);
	if (!r)
	...
	LOG("  0x%08x / 0x%08x = %u / %u = %u = 0x%x)",
	    (unsigned int)a, (unsigned int)b, (unsigned int)a, (unsigned int)b, (unsigned int)c,
	    (unsigned int)c);
	LOG("  0x%08x %% 0x%08x = %u %% %u = %u = 0x%x)", (unsigned int)a,
	    (unsigned int)b, (unsigned int)a, (unsigned int)b, (unsigned int)d, (unsigned int)d);
	LOG("  check results => %s", r ? "ok" : "FAILED !!!");
	LOG("");

	LOG("- test with signed small integers, negative numerator:");
	a = (signed int)(vaddr_t)&a;
	b = (signed int)((unsigned int)(vaddr_t)&b & 0x00FFF) - 1;
	c = a / b;
	d = a % b;
	r = ((b * c + d) == a);
