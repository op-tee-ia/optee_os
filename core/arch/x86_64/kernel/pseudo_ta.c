...
static TEE_Result verify_pseudo_tas_conformance(void)
{
	...
	for (pta = start; pta < end; pta++) {
		...
		IMSG("pta name %s, uuid=0x%x, 0x%x, 0x%x\n", pta->name,
				pta->uuid.timeLow, pta->uuid.timeMid,
				pta->uuid.timeHiAndVersion);
			...
			struct tee_ta_session *s)
{
	...
	while (true) {
		...
		ta = (struct pseudo_ta_head *)((uint8_t *)ta + 128);
	}
