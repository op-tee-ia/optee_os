  ...
  )
{
  ...
  switch (ResponseCode) {
    case TPM_RC_SUCCESS:
      DMSG("TPM2Startup: TPM_RC_SUCCESS\n");
      return EFI_SUCCESS;
    case TPM_RC_INITIALIZE:
      // TPM_RC_INITIALIZE can be returned if Tpm2Startup is not required.
      DMSG("TPM2Startup: TPM_RC_INITIALIZE\n");
      return EFI_SUCCESS;
    default:
      EMSG("Tpm2Startup: Response Code error! 0x%08x\r\n", ResponseCode);
  ...
  )
{
  ...
  if (SwapBytes32 (Res.Header.responseCode) != TPM_RC_SUCCESS) {
    EMSG("Tpm2Shutdown: Response Code error! 0x%08x\r\n", SwapBytes32 (Res.Header.responseCode));
    return EFI_DEVICE_ERROR;
  }
