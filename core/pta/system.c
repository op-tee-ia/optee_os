...
static TEE_Result system_set_rot(struct user_mode_ctx *uctx,
					      uint32_t param_types,
					      TEE_Param params[TEE_NUM_PARAMS])
{
	TEE_Result res = TEE_ERROR_GENERIC;
	...
	//TODO: will add multiple ivshmem devices handling
	res = ivshmem_rot_set(0, params[0].value.a,
				params[0].value.b);
				 ...
				 TEE_Param params[TEE_NUM_PARAMS])
{
	...
	case PTA_SYSTEM_SET_ROT:
		return system_set_rot(uctx, param_types, params);
#endif
